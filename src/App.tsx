...
export default function App() {
  ...
  useEffect(() => {
    ...
    const saved = localStorage.getItem('pointage_rh_clean_v11');
    if (saved) {
      try { setRecords(JSON.parse(saved)); } catch (e) { console.error(e); }
    }
    const savedCompany = localStorage.getItem('pointage_rh_company_v11');
    if (savedCompany) {
  ...
  const handlePointage = (type) => {
    ...
    // Capture GPS en arrière-plan sans bloquer l'interface
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        (position) => {
          const { latitude, longitude } = position.coords;
          saveRecord(type, `${latitude.toFixed(5)}, ${longitude.toFixed(5)}`);
        },
        (error) => {
          console.warn("GPS non disponible:", error.message);
          saveRecord(type, "GPS non disponible");
        },
        { timeout: 5000, enableHighAccuracy: true }
      );
    } else {
      saveRecord(type, "GPS non supporté");
    }
  };

  const saveRecord = (type, locationStr) => {
    const newRecord = {
      id: Date.now(),
      name: name.trim(),
      type,
    ...
    setRecords(updated);
    localStorage.setItem('pointage_rh_clean_v11', JSON.stringify(updated));
    
    // Le nom disparaît (champ vidé) et confirmation discrète
    setName('');
  ...
  const handleSaveCompany = (e) => {
    ...
    localStorage.setItem('pointage_rh_company_v11', newCompanyNameInput.trim());
  ...
  const confirmClearHistory = () => {
    setRecords([]);
    localStorage.removeItem('pointage_rh_clean_v11');
