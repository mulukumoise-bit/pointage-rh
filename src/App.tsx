...
export default function App() {
  ...
  const handlePointer = (type: string) => {
    ...
    // Fonction pour enregistrer le pointage avec des coordonnées
    ...
    // Coordonnées de secours (Lubumbashi) si le GPS échoue ou est refusé
    const fallbackLat = -11.57052 + (Math.random() * 0.002 - 0.001);
    const fallbackLng = 27.55102 + (Math.random() * 0.002 - 0.001);
    ...
    if ("geolocation" in navigator) {
      navigator.geolocation.getCurrentPosition(
        ...
        () => {
          // En cas de refus ou d'échec du GPS, on utilise le secours géolocalisé
        ...
        {!isAdminUnlocked ? (
          ...
          <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              {records.map(r => (
                <div key={r.id} style={{ backgroundColor: '#F8FAFC', padding: '12px', borderRadius: '10px', border: '1px solid #E2E8F0', fontSize: '11px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div>
                    <div style={{ fontWeight: 'bold', fontSize: '12px', color: '#1E293B', marginBottom: '3px' }}>{r.name}</div>
                    <div style={{ color: '#64748B', fontSize: '11px' }}>{r.time} -- {r.coords}</div>
                  </div>
                  <span style={{ padding: '4px 8px', borderRadius: '4px', fontSize: '10px', fontWeight: 'bold', backgroundColor: '#DCFCE7', color: '#166534' }}>
                    {r.type}
                  </span>
                </div>
              ))}
