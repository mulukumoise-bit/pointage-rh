...
export default function App() {
  ...
  const [records, setRecords] = useState(() => {
    const saved = localStorage.getItem('pointage_records_v4');
    if (saved) {
  ...
  useEffect(() => {
    localStorage.setItem('pointage_records_v4', JSON.stringify(records));
  }, [records]);
  ...
  const handlePointer = (type: string) => {
    ...
    // 1. VIDAGE INSTANTANÉ DE LA CASE ET CONFIRMATION
    setName('');
    setSuccessMessage(`✓ Pointage "${type}" enregistré pour ${trimmedName}`);
    ...
    // 2. ENREGISTREMENT (avec GPS en arrière-plan sans bloquer l'affichage)
    const addRecordToState = (lat: number, lng: number) => {
      const newRecord = {
        id: Date.now(),
        name: trimmedName,
        type,
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        position => addRecordToState(position.coords.latitude, position.coords.longitude),
        () => addRecordToState(fallbackLat, fallbackLng),
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#f4f6f9', fontFamily: 'Arial, sans-serif', padding: '16px', color: '#333' }}>
      <div style={{ maxWidth: '600px', margin: '0 auto' }}>
        ...
        {/* En-tête avec nom de l'entreprise et options admin */}
        ...
        {/* Bloc Formulaire de Pointage (Nom disparaît instantanément) */}
