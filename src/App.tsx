...
export default function App() {
  ...
  const [records, setRecords] = useState(() => {
    const saved = localStorage.getItem('pointage_records_v5');
    if (saved) {
  ...
  useEffect(() => {
    localStorage.setItem('pointage_records_v5', JSON.stringify(records));
  }, [records]);
  ...
  const handlePointer = (type: string) => {
    ...
    // Effacement immédiat de la case pour le prochain employé
    setName('');
    ...
    const saveToRegistry = (lat: number, lng: number) => {
      const newRecord = {
        id: Date.now(),
        name: trimmedName,
        type,
    ...
    // Tentative de récupération GPS rapide, avec repli immédiat garanti
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        position => saveToRegistry(position.coords.latitude, position.coords.longitude),
        () => saveToRegistry(fallbackLat, fallbackLng),
