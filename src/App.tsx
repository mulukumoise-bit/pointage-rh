...
export default function App() {
  ...
  useEffect(() => {
    ...
    const saved = localStorage.getItem('pointage_rh_final_v12');
    if (saved) {
      try { setRecords(JSON.parse(saved)); } catch (e) { console.error(e); }
    }
    const savedCompany = localStorage.getItem('pointage_rh_company_v12');
    if (savedCompany) {
  ...
  const handlePointage = (type) => {
    ...
    // Récupération GPS en arrière-plan
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        (position) => {
          const { latitude, longitude } = position.coords;
          saveRecord(type, trimmedName, `${latitude.toFixed(5)}, ${longitude.toFixed(5)}`);
        },
        (error) => {
          console.warn("GPS non disponible:", error.message);
          saveRecord(type, trimmedName, "GPS non disponible");
        },
        { timeout: 5000, enableHighAccuracy: true }
      );
    } else {
      saveRecord(type, trimmedName, "GPS non supporté");
    }
  };

  const saveRecord = (type, employeeName, locationStr) => {
    const newRecord = {
      id: Date.now(),
      name: employeeName,
      type,
    ...
    setRecords(updated);
    localStorage.setItem('pointage_rh_final_v12', JSON.stringify(updated));
    
    // Le nom disparaît immédiatement (champ vidé) pour confirmer l'enregistrement
    setName('');
  ...
  const handleSaveCompany = (e) => {
    ...
    localStorage.setItem('pointage_rh_company_v12', newCompanyNameInput.trim());
  ...
  const confirmClearHistory = () => {
    setRecords([]);
    localStorage.removeItem('pointage_rh_final_v12');
