...
export default function App() {
  ...
  // Gestion de la date d'expiration (Stockage Local + Calcul)
  ...
  // Écouter l'historique de l'entreprise active
  useEffect(() => {
  ...
  const ValiderAbonnement = (e: React.FormEvent) => {
    ...
    if (CODES_ABONNEMENT_VALIDES.includes(codeClean)) {
      ...
      notifier('Abonnement Prolongé', 'Votre accès a été renouvelé pour 30 jours supplémentaires !', 'succes');
    } else {
      notifier('Code Invalide', 'Code de recharge incorrect. Contactez le service client.', 'erreur');
    }
  };
  ...
  const GenererLienEmploye = () => {
    ...
    notifier('Lien Copié !', `Lien unique copié pour l'entreprise ${nomEntreprise} :\n${url}`, 'succes');
  };
  ...
  // Vider la corbeille (Correction directe document par document)
  const ViderHistoriqueCloud = async () => {
    try {
      ...
      notifier('Corbeille Vidée', `Tous les pointages de ${nomEntreprise} ont été supprimés avec succès.`, 'succes');
    } catch (err) {
      setModaleConfirmationVidage(false);
      notifier('Erreur', 'Impossible de vider la corbeille.', 'erreur');
    }
  };
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#f8fafc', color: '#0f172a', fontFamily: 'Inter, system-ui, sans-serif' }}>
      ...
      {/* HEADER AVEC BADGE D'ABONNEMENT CLAIR */}
      <header style={{ backgroundColor: '#0f172a', color: '#fff', padding: '16px 20px' }}>
        <div style={{ maxWidth: '600px', margin: '0 auto', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <div style={{ backgroundColor: joursRestants > 5 ? '#16a34a' : '#dc2626', color: '#ffffff', padding: '6px 14px', borderRadius: '20px', fontSize: '13px', fontWeight: '700', border: '1px solid rgba(255,255,255,0.2)' }}>
                ...
                {nomEntreprise && (
                  <>
                    ...
                    <div style={{ display: 'flex', gap: '10px', marginBottom: '16px' }}>
                      <button onClick={CopierRapport} style={{ flex: 1, padding: '14px', backgroundColor: '#2563eb', color: '#fff', border: 'none', borderRadius: '10px', fontWe
