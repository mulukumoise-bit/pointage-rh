...
export default function App() {
  ...
  // Export intelligent et propre sans popup "Save As" bloquant
  const executeExport = async () => {
    ...
    const finalName = exportFileName.trim() ? `${exportFileName.trim()}.csv` : `pointages.csv`;

    const blob = new Blob([csvData], { type: 'text/csv;charset=utf-8;' });
    const file = new File([blob], finalName, { type: 'text/csv' });

    // Utilisation de l'API de partage native du téléphone (si dispo) -> Évite totalement le "Save As"
    if (navigator.canShare && navigator.canShare({ files: [file] })) {
      try {
        await navigator.share({
          files: [file],
          title: 'Export Pointages RH',
          text: 'Voici le fichier des pointages au format Excel (CSV).'
        });
        setShowExportModal(false);
        return;
      } catch (err) {
        if ((err as Error).name !== 'AbortError') {
          console.log('Partage annulé ou non pris en charge, utilisation de la méthode alternative');
        } else {
    ...
    // Fallback propre : Affichage direct du texte CSV dans la modale pour copie ou téléchargement sécurisé
    setCsvContentText(csvData);
  };

  const downloadCsvDirect = () => {
    const blob = new Blob([csvContentText], { type: 'text/csv;charset=utf-8;' });
    const url = URL.createObjectURL(blob);
    const link = document.createElement("a");
    link.href = url;
    link.download = `${exportFileName.trim() || 'pointages'}.csv`;
          ...
          <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0, 0, 0, 0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: '16px' }}>
            <div style={{ backgroundColor: '#ffffff', padding: '20px', borderRadius: '12px', maxWidth: '380px', width: '100%', boxShadow: '0 4px 12px rgba(0,0,0,0.15)' }}>
              ...
              <div style={{ textAlign: 'center', marginBottom: '14px' }}>
                ...
                <h3 style={{ fontSize: '16px', margin: 0, color: '#0f3d3e' }}>Exporter le registre</h3>
                <p style={{ fontSize: '12px', color: '#64748b', margin: '4px 0 0 0' }}>
                  {records.length} enregistrement(s) prêt(s).
                </p>
              </div>

              {csvContentText ? (
                <div>
                  <p style={{ fontSize: '12px', color: '#334155', marginBottom: '6px', fontWeight: 'bold' }}>
                    Copiez ou téléchargez vos données ci-dessous :
                  </p>
                  <textarea
                    readOnly
                    value={csvContentText}
                    style={{ width: '100%', height: '100px', fontSize: '11px', padding: '8px', borderRadius: '6px', border: '1px solid #cbd5e1', marginBot
