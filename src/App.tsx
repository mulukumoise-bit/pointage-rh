...
export default function App() {
  ...
  useEffect(() => {
    ...
    const saved = localStorage.getItem('pointage_rh_final_v8');
    if (saved) {
      try { setRecords(JSON.parse(saved)); } catch (e) { console.error(e); }
    }
    const savedCompany = localStorage.getItem('pointage_rh_company_v8');
    if (savedCompany) {
  ...
  const handlePointage = (type) => {
    ...
    setRecords(updated);
    localStorage.setItem('pointage_rh_final_v8', JSON.stringify(updated));
    setName('');
  ...
  const handleSaveCompany = (e) => {
    ...
    localStorage.setItem('pointage_rh_company_v8', newCompanyNameInput.trim());
  ...
  const confirmClearHistory = () => {
    setRecords([]);
    localStorage.removeItem('pointage_rh_final_v8');
                    ...
                    <tbody>
                      {records.map((r, index) => (
                        <tr key={r.id} style={{ backgroundColor: index % 2 === 0 ? '#ffffff' : '#f5f2eb', borderBottom: '1px solid #e7e2d6' }}>
                          ...
                          <td style={{ padding: '10px', color: '#059669', fontSize: '10px', fontFamily: 'monospace' }}>{r.location}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
