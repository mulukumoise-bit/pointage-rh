...
export default function App() {
  ...
  const InscrireEntreprise = async (e: React.FormEvent) => {
    ...
    try {
      ...
      notifier('Entreprise Créée !', `L'entreprise "${nomClean}" est créée.`, 'succes');
    } catch (err) {
  ...
  const EnregistrerPointage = (type: 'Arrivée' | 'Départ') => {
    if (!nomEntreprise) {
      notifier('Entreprise requise', 'Veuillez sélectionner une entreprise.', 'erreur');
      return;
    }
    if (!nomEmploye.trim()) {
      notifier('Nom requis', 'Veuillez saisir votre Nom et Prénom.', 'erreur');
      return;
    }

    if (Date.now() > dateExpiration) {
      notifier('Abonnement Expiré', 'Abonnement de 30 jours expiré.', 'erreur');
      return;
    }
  ...
  const SauvegarderPointageFirebase = async (type: 'Arrivée' | 'Départ', gpsStr: string, estDansZone: boolean, dist: number) => {
    ...
    try {
      ...
      notifier('Pointage Enregistré', `Pointage d'${type} validé pour ${userSaved}.`, 'succes');
    } catch (err) {
      setChargementGPS(false);
  ...
  const ValiderAbonnement = (e: React.FormEvent) => {
    ...
    if (CODES_ABONNEMENT_VALIDES.includes(codeClean)) {
      ...
      notifier('Abonnement Activé', 'Recharge réussie !', 'succes');
    } else {
      notifier('Code Invalide', 'Code incorrect.', 'erreur');
    }
  };
  ...
  const GenererLienEmploye = () => {
    ...
    notifier('Lien Copié !', url, 'succes');
  };

  const CopierRapport = () => {
    if (historique.length === 0) {
      notifier('Vide', 'Aucun pointage.', 'info');
      return;
    }
    ...
    notifier('Copié !', 'Rapport copié.', 'succes');
  };
  ...
  const ExecutionReinitialisation = async () => {
    try {
      ...
      notifier('Réussi', 'Historique réinitialisé.', 'succes');
    } catch (err) {
      setModaleConfirmationReinitialisation(false);
      notifier('Erreur', 'Impossible de réinitialiser.', 'erreur');
    }
  };
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#f8fafc', color: '#0f172a', fontFamily: 'sans-serif' }}>
      ...
      <div style={{ display: 'flex', backgroundColor: '#fff', borderBottom: '1px solid #e2e8f0' }}>
        <button onClick={() => setOnglets('pointage')} style={{ flex: 1, padding: '12px', border: 'none', background: 'none', fontWeight: 'bold', color: onglets === 'pointage' ? '#2563eb' : '#64748b', borderBottom: onglets === 'pointage' ? '2px solid #2563eb' : 'none' }}>Pointage</button>
        <button onClick={() => setOnglets('admin')} style={{ flex: 1, padding: '12px', border: 'none', background: 'none', fontWeight: 'bold', color: onglets === 'admin' ? '#2563eb' : '#64748b', borderBottom: onglets === 'admin' ? '2px solid #2563eb' : 'none' }}>Admin</button>
        <button onClick={() => setOnglets('abonnement')} style={{ flex: 1, padding: '12px', border: 'none', background: 'none', fontWeight: 'bold', color: onglets === 'abonnement' ? '#2563eb' : '#64748b', borderBottom: onglets === 'abonnement' ? '2px solid #2563eb' : 'none' }}>Service</button>
      </div>
      ...
      <main style={{ padding: '16px', maxWidth: '600px', margin: '0 auto' }}>
          ...
          <div>
            ...
            <div style={{ background: '#fff', padding: '16px', borderRadius: '12px', marginBottom: '16px', border: '1px solid #e2e8f0' }}>
              ...
              <div style={{ display: 'flex', gap: '10px' }}>
                <button disabled={chargementGPS} onClick={() => EnregistrerPointage('Arrivée')} style={{ flex: 1, padding: '14px', background: '#16a34a', color: '#fff', border: 'none', borderRadius: '8px', fontWeight: 'bold' }}>ARRIVÉE</button>
                <button disabled={chargementGPS} onClick={() => EnregistrerPointage('Départ')} style={{ flex: 1, padding: '14px', background: '#dc2626', color: '#fff', border: 'none', borderRadius: '8px', fontWeight: 'bold' }}>DÉPART</button>
              </div>
            </div>
              ...
              <div style={{ background: '#fff', padding: '16px', borderRadius: '12px', border: '1px solid #e2e8f0' }}>
                ...
                {historique.length === 0 ? <p style={{ fontSize: '13px', color: '#94a3b8' }}>Aucun pointage.</p> : (
                  historique.slice(0, 5).map(item => (
                    <div key={item.id} style={{ padding: '8px 0', borderBottom: '1px solid #f1f5f9', display: 'flex', justifyContent: 'space-between' }}>
                      <div>
            ...
            {!estAdminAuthentifie ? (
              <div style={{ background: '#fff', padding: '16px', borderRadius: '12px', border: '1px solid #e2e8f0' }}>
                ...
                <form onSubmit={(e) => { e.preventDefault(); codeAdminInput === 'admin123' ? setEstAdminAuthentifie(true) : setErreurAdmin('Incorrect'); }}>
                  ...
                  <button type="submit" style={{ width: '100%', padding: '10px', background: '#0f172a', color: '#fff', border: 'none', borderRadius: '8px', fontWeight: 'bold' }}>Valider</button>
                </form>
              </div>
            ) : (
              <div>
                <div style={{ background: '#fff', padding: '16px', borderRadius: '12px', marginBottom: '16px', border: '1px solid #e2e8f0' }}>
                  ...
                  <form onSubmit={InscrireEntreprise} style={{ display: 'flex', gap: '8px' }}>
                    <input type="text" placeholder="Nom" value={nouvelleEntrepriseInput} onChange={(e) => setNouvelleEntrepriseInput(e.target.value)} style={{ flex: 1, padding: '8px', borderRadius: '8px', border: '1px solid #cbd5e1' }} />
                    <button type="submit" style={{ padding: '8px 12px', background: '#2563eb', color: '#fff', border: 'none', borderRadius: '8px', fontWeight: 'bold' }}>Créer</button>
                  </form>
                </div>

                {nomEntreprise && (
                  <div>
                    <button onClick={GenererLienEmploye} style={{ width: '100%', padding: '12px', background: '#16a34a', color: '#fff', border: 'none', borderRadius: '8px', fontWeight: 'bold', marginBottom: '10px' }}>🔗 Copier le lien employés</button>
                    <div style={{ display: 'flex', gap: '10px', marginBottom: '16px' }}>
                      <button onClick={CopierRapport} style={{ flex: 1, padding: '10px', background: '#2563eb', color: '#fff', border: 'none', borderRadius: '8px', fontWeight: 'bold' }}>📋 Rapport</button>
                      <button onClick={() => setModaleConfirmationReinitialisation(true)} style={{ flex: 1, padding: '10px', background: '#dc2626', color: '#fff', border: 'none', borderRadius: '8px', fontWeight: 'bold' }}>🗑️ Vider</button>
                    </div>
                  </div>
                )}
              </div>
        ...
        {onglets === 'abonnement' && (
          <div style={{ background: '#fff', padding: '16px', borderRadius: '12px', border: '1px solid #e2e8f0' }}>
            <h2 style={{ fontSize: '16px', marginTop: 0 }}>Abonnement 30 Jours</h2>
            <p style={{ fontSize: '13px', color: '#475569' }}>Airtel Money : <strong>{NUMERO_AIRTEL}</strong></p>
            <form onSubmit={ValiderAbonnement} style={{ marginTop: '16px' }}>
              ...
              <button type="submit" style={{ width: '100%', padding: '10px', background: '#0f172a', color: '#fff', border: 'none', borderRadius: '8px', fontWeight: 'bold' }}>Activer</button>
            </form>
          </div>
        )}
      </main>

      {modaleConfirmationReinitialisation && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px' }}>
          <div style={{ background: '#fff', padding: '20px', borderRadius: '12px', maxWidth: '300px', width: '100%' }}>
            ...
            <div style={{ display: 'flex', gap: '10px', marginTop: '16px' }}>
              <button onClick={() => setModaleConfirmationReinitialisation(false)} style={{ flex: 1, padding: '10px', background: '#e2e8f0', border: 'none', borderRadius: '8px' }}>Annuler</button>
              <button onClick={ExecutionReinitialisation} style={{ flex: 1, padding: '10px', background: '#dc2626', color: '#fff', border: 'none', borderRadius: '8px' }}>Oui</button>
            </div>
          </div>
        </div>
      )}
      ...
      {modaleInfo.ouverte && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px' }}>
          <div style={{ background: '#fff', padding: '20px', borderRadius: '12px', maxWidth: '300px', width: '100%' }}>
            <h3 style={{ marginTop: 0 }}>{modaleInfo.titre}</h3>
            <p style={{ fontSize: '13px', color: '#475569' }}>{modaleInfo.text}</p>
            <button onClick={() => setModaleInfo({ ...modaleInfo, ouverte: false })} style={{ width: '100%', padding: '10px', background: '#0f172a', color: '#fff', border: 'none', borderRadius: '8px' }}>OK</button>
          </div>
        </div>
      )}
    </div>
