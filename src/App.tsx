...
export default function App() {
  ...
  useEffect(() => {
    ...
    const saved = localStorage.getItem('pointage_rh_gps_pro_v10');
    if (saved) {
      try { setRecords(JSON.parse(saved)); } catch (e) { console.error(e); }
    }
    const savedCompany = localStorage.getItem('pointage_rh_company_v10');
    if (savedCompany) {
  ...
  const handlePointage = (type) => {
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        const locationStr = `${latitude.toFixed(5)}, ${longitude.toFixed(5)}`;
        setIsLoadingGps(false);
        saveRecordWithLocation(type, locationStr);
      },
      (error) => {
        console.warn("Erreur GPS:", error.code, error.message);
        setIsLoadingGps(false);
        // Fallback propre en cas de refus ou d'échec du signal GPS
        let errorMsg = "Position non disponible";
        if (error.code === 1) errorMsg = "GPS refusé par l'utilisateur";
        else if (error.code === 2) errorMsg = "Signal GPS indisponible";
        else if (error.code === 3) errorMsg = "Délai GPS dépassé";
        
        saveRecordWithLocation(type, errorMsg);
      },
  ...
  const saveRecordWithLocation = (type, locationStr) => {
    ...
    setRecords(updated);
    localStorage.setItem('pointage_rh_gps_pro_v10', JSON.stringify(updated));
    setName('');
  ...
  const handleSaveCompany = (e) => {
    ...
    localStorage.setItem('pointage_rh_company_v10', newCompanyNameInput.trim());
  ...
  const confirmClearHistory = () => {
    setRecords([]);
    localStorage.removeItem('pointage_rh_gps_pro_v10');
        ...
        <div style={{ backgroundColor: '#faf8f5', padding: '24px 20px', borderRadius: '20px', marginBottom: '20px', boxShadow: '0 4px 15px rgba(0,0,0,0.03)', border: '1px solid #e7e2d6' }}>
          ...
          <div style={{ display: 'flex', gap: '12px' }}>
            ...
            >
              ...
              {isLoadingGps ? 'Recherche...' : 'Arrivée'}
            </button>
            <button
              type="button"
              disabled={isLoadingGps}
