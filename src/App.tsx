...
export default function App() {
  ...
  // Gestion de la date d'expiration (Stockage Local + Calcul 30 jours)
  ...
  // Détection de l'entreprise via l'URL ou le LocalStorage
  useEffect(() => {
  ...
  // Écouter la liste des entreprises
  useEffect(() => {
  ...
  // Écouter l'historique des pointages
  useEffect(() => {
  ...
  // Calcul de la distance GPS (Haversine)
  ...
  const SauvegarderPointageFirebase = async (type: 'Arrivée' | 'Départ', gpsStr: string, estDansZone: boolean, dist: number) => {
    ...
    try {
      ...
      notifier(
        'Pointage Enregistré',
        `Pointage d'${type} validé pour ${userSaved}.\nGPS : ${estDansZone ? '🟢 Dans la zone' : `🔴 Hors zone (${dist}m)`}`,
        'succes'
      );
    } catch (err) {
      setChargementGPS(false);
  ...
  const ExecutionReinitialisation = async () => {
    try {
      ...
      notifier('Réinitialisation Reussie', `L'historique de ${nomEntreprise} a été réinitialisé.`, 'succes');
    } catch (err) {
      setModaleConfirmationReinitialisation(false);
      notifier('Erreur', 'Impossible de réinitialiser l\'historique.', 'erreur');
    }
  };
    ...
    <div style={{ minHeight: '100vh', backgroundColor: '#f8fafc', color: '#0f172a', fontFamily: 'Inter, system-ui, sans-serif' }}>
      ...
      <header style={{ backgroundColor: '#0f172a', color: '#fff', padding: '16px 20px' }}>
        <div style={{ maxWidth: '600px', margin: '0 auto', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <div style={{ backgroundColor: joursRestants > 5 ? '#16a34a' : '#dc2626', color: '#ffffff', padding: '6px 14px', borderRadius: '20px', fontSize: '13px', fontWeight: '700' }}>
          ...
          <div>
            ...
            {nomEntreprise && (
              <div style={{ backgroundColor: '#ffffff', borderRadius: '16px', padding: '20px', border: '1px solid #e2e8f0' }}>
                ...
                {historique.length === 0 ? (
                  <p style={{ color: '#94a3b8', fontSize: '14px' }}>Aucun pointage enregistré.</p>
                ) : (
                    ...
                    {historique.slice(0, 5).map((item) => (
                      <div key={item.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px', backgroundColor: '#f8fafc', borderRadius: '10px', border: '1px solid #f1f5f9' }}>
                        <div>
                          ...
                          <div style={{ fontSize: '12px', color: '#64748b', marginTop: '2px' }}>{item.date} à {item.heure}</div>
                          <div style={{ fontSize: '12px', color: item.estDansLaZone ? '#16a34a' : '#dc2626', fontWeight: '700', marginTop: '4px' }}>
                            {item.estDansLaZone ? '🟢 Dans la zone GPS' : `🔴 Hors zone (${item.distanceMetres}m)`}
                          </div>
                        </div>
                        <span style={{ padding: '6px 10px', borderRadius: '12px', fontSize: '12px', fontWeight: '700', backgroundColor: item.type === 'Arrivée' ? '#dcfce7' : '#fee2e2', color: item.type === 'Arrivée' ? '#15803d' : '#b91c1c' }}>
                          {item.type}
                        </span>
                      </div>
                    ))}
            ...
            {!estAdminAuthentifie ? (
              <div style={{ backgroundColor: '#ffffff', borderRadius: '16px', padding: '24px', border: '1px solid #e2e8f0' }}>
                ...
                <form onSubmit={(e) => { e.preventDefault(); codeAdminInput === 'admin123' ? setEstAdminAuthentifie(true) : setErreurAdmin('Code incorrect'); }}>
                  ...
                  <button type="submit" style={{ width: '100%', padding: '12px', backgroundColor: '#0f172a', color: '#fff', border: 'none', borderRadius: '8px', fontWeight: '700', cursor: 'pointer' }}>Se Connecter</button>
                </form>
              </div>
            ) : (
              <div>
                <div style={{ backgroundColor: '#ffffff', borderRadius: '16px', padding: '20px', border: '1px solid #e2e8f0', marginBottom: '20px' }}>
                  ...
                  <form onSubmit={InscrireEntreprise} style={{ display: 'flex', gap: '8px' }}>
                    <input
                      type="text"
                      placeholder="Nom (Ex: SARL)"
                      value={nouvelleEntrepriseInput}
                      onChange={(e) => setNouvelleEntrepriseInput(e.target.value)}
                      style={{ flex: 1, padding: '10px', borderRadius: '8px', border: '1px solid #cbd5e1' }}
                    />
                    <button type="submit" style={{ padding: '10px 16px', backgroundColor: '#2563eb', color: '#fff', border: 'none', borderRadius: '8px', fontWeight: '700', cursor: 'pointer' }}>Créer</button>
                  </form>
                </div>
                ...
                {nomEntreprise && (
                  <>
                    ...
                    <div style={{ display: 'flex', gap: '10px', marginBottom: '16px' }}>
                      ...
                      <button onClick={() => setModaleConfirmationReinitialisation(true)} st
