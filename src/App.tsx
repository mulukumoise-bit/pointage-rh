...
export default function App() {
  ...
  useEffect(() => {
    ...
    const saved = localStorage.getItem('pointage_rh_v13');
    if (saved) {
      try { setRecords(JSON.parse(saved)); } catch (e) { console.error(e); }
    }
    const savedCompany = localStorage.getItem('pointage_rh_company_v13');
  ...
  const executeSave = (type, employeeName, locationStr) => {
    const newRecord = {
      id: Date.now(),
      name: employeeName,
      type,
    ...
    setRecords(updated);
    localStorage.setItem('pointage_rh_v13', JSON.stringify(updated));
    
    setName('');
    showToast(`Pointage (${type}) enregistré avec succès !`);
  };
  ...
  const handlePointage = (type) => {
    ...
    // Sécurité anti-blocage : Si le GPS met plus de 3 secondes ou échoue, on enregistre quand même !
    ...
    if (navigator.geolocation) {
      const timeoutId = setTimeout(() => {
        if (!gpsHandled) {
          gpsHandled = true;
          executeSave(type, trimmedName, "GPS non disponible (délai dépassé)");
        }
      }, 3000);
      ...
      navigator.geolocation.getCurrentPosition(
        (position) => {
          if (!gpsHandled) {
            ...
            executeSave(type, trimmedName, `${latitude.toFixed(5)}, ${longitude.toFixed(5)}`);
          }
        },
        (error) => {
          if (!gpsHandled) {
            gpsHandled = true;
            clearTimeout(timeoutId);
            executeSave(type, trimmedName, "GPS désactivé/refusé");
          }
        },
        { timeout: 3000, enableHighAccuracy: false, maximumAge: 10000 }
      );
    } else {
      executeSave(type, trimmedName, "GPS non supporté");
    }
  };
  ...
  const handleSaveCompany = (e) => {
    ...
    localStorage.setItem('pointage_rh_company_v13', newCompanyNameInput.trim());
  ...
  const handleCopyReport = () => {
    if (records.length === 0) {
      showToast("Aucun historique.", "error");
      return;
    }
  ...
  const confirmClearHistory = () => {
    setRecords([]);
    localStorage.removeItem('pointage_rh_v13');
      ...
      <div style={{ maxWidth: '480px', margin: '0 auto' }}>
        ...
        <div style={{ background: 'linear-gradient(135deg, #1d4ed8 0%, #3b82f6 50%, #60a5fa 100%)', color: '#ffffff', padding: '24px 20px', borderRadius: '20px', marginBottom: '20px', boxShadow: '0 10px 25px rgba(59, 130, 246, 0.3)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <button
            ...
            style={{ backgroundColor: 'rgba(255, 255, 255, 0.2)', border: '1px solid rgba(255, 255, 255, 0.4)', color: '#ffffff', padding: '10px 14px', borderRadius: '12px', fontSize: '12px', fontWeight: '700', cursor: 'pointer', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', gap: '6px' }}
          >
        ...
        <div style={{ backgroundColor: '#faf8f5', padding: '20px', borderRadius: '20px', marginBottom: '20px', textAlign: 'center', boxShadow: '0 4px 15px rgba(0,0,0,0.03)', border: '1px solid #e7e2d6' }}>
          ...
          <div style={{ marginTop: '8px', fontSize: '11px', color: '#059669', fontWeight: '700' }}>
            📍 Le GPS est enregistré automatiquement au clic
          </div>
        </div>
        ...
        <div style={{ backgroundColor: '#faf8f5', padding: '24px 20px', borderRadius: '20px', marginBottom: '20px', boxShadow: '0 4px 15px rgba(0,0,0,0.03)', border: '1px solid #e7e2d6' }}>
          ...
          <div style={{ display: 'flex', gap: '12px' }}>
            <button
              ...
              style={{ flex: 1, padding: '14px', borderRadius: '12px', border: 'none', fontWeight: '700', fontSize: '14px', cursor: 'pointer', backgroundColor: '#10b981', color: '#ffffff', boxShadow: '0 4px 12px rgba(16, 185, 129, 0.25)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}
            >
            ...
            <button
              ...
              style={{ flex: 1, padding: '14px', borderRadius: '12px', border: 'none', fontWeight: '700', fontSize: '14px', cursor: 'pointer', backgroundColor: '#ef4444', color: '#ffffff', boxShadow: '0 4px 12px rgba(239, 68, 68, 0.25)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}
            >
              <span style={{ width: '8px', height: '8px', backgroundColor: '#ffffff', borderRadius: '50%' }}></span>
              Départ
            </button>
          ...
          {!isAdminUnlocked ? (
            ...
            <div>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                ...
                <button onClick={handleCopyReport} style={{ backgroundColor: '#10b981', color: '#ffffff', border: 'none', padding: '8px 12px', borderRadius: '8px', fontWeight: '700', fontSize: '12px', cursor: 'pointer' }}>
                  📋 Copier le rapport
                </button>
              </div>
