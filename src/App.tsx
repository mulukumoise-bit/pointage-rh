...
export default function App() {
  ...
  const InscrireEntreprise = async (e: React.FormEvent) => {
    ...
    try {
      ...
      notifier('Succès', `L'entreprise "${nomClean}" est créée.`, 'succes');
    } catch (err) {
  ...
  const EnregistrerPointage = (type: 'Arrivée' | 'Départ') => {
    if (!nomEntreprise) {
      notifier('Entreprise requise', 'Veuillez sélectionner une entreprise dans la liste.', 'erreur');
      return;
    }
    if (!nomEmploye.trim()) {
    ...
    if (Date.now() > dateExpiration) {
      notifier('Abonnement Expiré', 'Abonnement de 30 jours expiré. Rendez-vous dans l\'onglet Service.', 'erreur');
      return;
    }

    setChargementGPS(true);

    let gpsEnregistre = false;

    const actionSauvegarde = async (lat?: number, lng?: number) => {
      if (gpsEnregistre) return;
      gpsEnregistre = true;

      let dansLaZone = true;
      let dist = 0;
      let gpsStr = "GPS Désactivé / Ignoré";
      ...
      try {
        ...
        notifier(
          'Pointage Enregistré !',
          `Pointage d'${type} validé pour ${userSaved}.\n${dansLaZone ? '🟢 Dans la zone autorisée' : `🔴 Hors zone (${dist}m)`}`,
          'succes'
        );
      } catch (err) {
        setChargementGPS(false);
        notifier('Erreur', 'Impossible d\'enregistrer dans le Cloud.', 'erreur');
      }
    };

    // Sécurité anti-blocage : Si le GPS met plus de 4 secondes, on force l'enregistrement
    const securiteTemps = setTimeout(() => {
      actionSauvegarde();
    }, 4000);
    ...
    if ('geolocation' in navigator) {
      navigator.geolocation.getCurrentPosition(
        (position) => {
          clearTimeout(securiteTemps);
          actionSauvegarde(position.coords.latitude, position.coords.longitude);
        },
        () => {
          clearTimeout(securiteTemps);
          actionSauvegarde(); // Enregistre quand même sans GPS
        },
  ...
  const ValiderAbonnement = (e: React.FormEvent) => {
    ...
    if (CODES_ABONNEMENT_VALIDES.includes(codeClean)) {
      ...
      notifier('Abonnement Activé', 'Votre abonnement de 30 jours a été renouvelé avec succès !', 'succes');
    } else {
      notifier('Code Invalide', 'Code de recharge incorrect.', 'erreur');
    }
  };
  ...
  const GenererLienEmploye = () => {
    ...
    notifier('Lien Copié !', `Lien pour ${nomEntreprise} :\n${url}`, 'succes');
  };

  const CopierRapport = () => {
    if (historique.length === 0) {
      notifier('Vide', 'Aucun pointage à copier.', 'info');
      return;
    }
    ...
    notifier('Copié !', 'Rapport copié dans le presse-papier.', 'succes');
  };
          ...
          <div>
            ...
            <div style={{ background: '#fff', padding: '16px', borderRadius: '12px', marginBottom: '16px', border: '1px solid #e2e8f0' }}>
              ...
              <div style={{ display: 'flex', gap: '10px' }}>
                <button disabled={chargementGPS} onClick={() => EnregistrerPointage('Arrivée')} style={{ flex: 1, padding: '14px', background: '#16a34a', color: '#fff', border: 'none', borderRadius: '8px', fontWeight: 'bold', cursor: 'pointer', opacity: chargementGPS ? 0.6 : 1 }}>
                  {chargementGPS ? 'Validation...' : 'ARRIVÉE'}
                </button>
                <button disabled={chargementGPS} onClick={() => EnregistrerPointage('Départ')} style={{ flex: 1, padding: '14px', background: '#dc2626', color: '#fff', border: 'none', borderRadius: '8px', fontWeight: 'bold', cursor: 'pointer', opacity: chargementGPS ? 0.6 : 1 }}>
                  {chargementGPS ? 'Validation...' : 'DÉPART'}
                </button>
              </div>
            </div>
          ...
          <div>
            {!estAdminAuthentifie ? (
              ...
              <div>
                ...
                {nomEntreprise && (
                  <div>
                    <button onClick={GenererLienEmploye} style={{ width: '100%', padding: '12px', background: '#16a34a', color: '#fff', border: 'none', borderRadius: '8px', fontWeight: 'bold', marginBottom: '10px', cursor: 'pointer' }}>🔗 Copier le lien employés</button>
                    <div style={{ display: 'flex', gap: '10px', marginBottom: '16px' }}>
                      <button onClick={CopierRapport} style={{ flex: 1, padding: '10px', background: '#2563eb', color: '#fff', border: 'none', borderRadius: '8px', fontWeight: 'bold', cursor: 'pointer' }}>📋 Rapport</button>
                      <button onClick={() => setModaleConfirmationReinitialisation(true)} style={{ flex: 1, padding: '10px', background: '#dc2626', color: '#fff', border: 'none', borderRadius: '8px', fontWeight: 'bold', cursor: 'pointer' }}>🗑️ Vider</button>
                    </div>
                  </div>
                )}
              </div>
        ...
        {onglets === 'abonnement' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            <div style={{ background: '#fff', padding: '16px', borderRadius: '12px', border: '1px solid #e2e8f0' }}>
              ...
              <p style={{ fontSize: '13px', color: '#475569', marginBottom: '16px' }}>Contactez-nous directement sur WhatsApp pour l'obtention de vos codes d'activation :</p>
            ...
            <div style={{ background: '#fff', padding: '16px', borderRadius: '12px', border: '1px solid #e2e8f0' }}>
              ...
              <form onSubmit={ValiderAbonnement} style={{ marginTop: '12px' }}>
                <input type="text" placeholder="Code (ex: RH2026-A1X9)" value={codeSaisiAbonnement} onChange={(e) => setCodeSaisiAbonnement(e.target.value)} style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1px solid #cbd5e1', mar
