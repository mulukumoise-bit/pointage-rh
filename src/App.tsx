...
export default function App() {
  ...
  useEffect(() => {
    ...
    const saved = localStorage.getItem('pointage_rh_exact_v6');
    if (saved) {
      try { setRecords(JSON.parse(saved)); } catch (e) { console.error(e); }
    }
    const savedCompany = localStorage.getItem('pointage_rh_company_v6');
    if (savedCompany) {
  ...
  const handlePointage = (type) => {
    ...
    setRecords(updated);
    localStorage.setItem('pointage_rh_exact_v6', JSON.stringify(updated));
    setName('');
  ...
  const handleSaveCompany = (e) => {
    ...
    localStorage.setItem('pointage_rh_company_v6', newCompanyNameInput.trim());
  ...
  const confirmClearHistory = () => {
    setRecords([]);
    localStorage.removeItem('pointage_rh_exact_v6');
    setShowClearModal(false);
    showToast("Historique réinitialisé.");
  };

  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#f0f4f8', fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif', padding: '16px', boxSizing: 'border-box', color: '#1e293b' }}>
      <div style={{ maxWidth: '480px', margin: '0 auto' }}>
        ...
        {/* Top Header avec Titre et Étoile */}
        ...
        {/* Bloc Date & Heure Actuelle */}
        <div style={{ backgroundColor: '#ffffff', padding: '20px', borderRadius: '20px', marginBottom: '20px', textAlign: 'center', boxShadow: '0 4px 15px rgba(0,0,0,0.03)', border: '1px solid #e2e8f0' }}>
        ...
        {/* Bloc Identification & Pointage */}
        <div style={{ backgroundColor: '#ffffff', padding: '24px 20px', borderRadius: '20px', marginBottom: '20px', boxShadow: '0 4px 15px rgba(0,0,0,0.03)', border: '1px solid #e2e8f0' }}>
        ...
        {/* Espace Direction / RH Sécurisé */}
        <div style={{ backgroundColor: '#ffffff', padding: '24px 20px', borderRadius: '20px', marginBottom: '30px', boxShadow: '0 4px 15px rgba(0,0,0,0.03)', border: '1px solid #e2e8f0' }}>
          {!isAdminUnlocked ? (
            <div>
              ...
              <form onSubmit={handleAdminLoginSubmit} style={{ display: 'flex', gap: '10px' }}>
                <input
                  ...
                  style={{ flex: 1, padding: '12px 14px', borderRadius: '10px', border: '1.5px solid #cbd5e1', fontSize: '13px', outline: 'none', backgroundColor: '#f8fafc' }}
                />
            ...
            <div>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                <div>
                  <h3 style={{ fontSize: '15px', margin: '0', color: '#1e293b', fontWeight: '800' }}>📊 Registre des Présences</h3>
      ...
      {isEditingCompany && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(15, 23, 42, 0.6)', backdropFilter: 'blur(4px)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 1000, padding: '16px' }}>
          <div style={{ backgroundColor: '#ffffff', padding: '24px', borderRadius: '20px', maxWidth: '380px', width: '100%', boxShadow: '0 20px 25px rgba(0,0,0,0.15)' }}>
            <h3 style={{ fontSize: '18px', color: '#0f172a', margin: '0 0 6px 0', fontWeight: '800' }}>Nom de l'entreprise</h3>
            <p style={{ fontSize: '13px', color: '#64748b', margin: '0 0 16px 0' }}>Personnalisez l'en-tête de votre application.</p>
            <form onSubmit={handleSaveCompany}>
              <input
                ...
                style={{ width: '100%', padding: '12px 14px', borderRadius: '10px', border: '1.5px solid #cbd5e1', marginBottom: '20px', fontSize: '14px', boxSizing: 'border-box', outline: 'none', backgroundColor: '#f8fafc' }}
              />
              <div style={{ display: 'flex', gap: '10px' }}>
                <button type="button" onClick={() => setIsEditingCompany(false)} style={{ flex: 1, padding: '12px', borderRadius: '10px', border: '1px solid #cbd5e1', backgroundColor: '#ffffff', color: '#334155', fontWeight: '700', fontSize: '13px', cursor: 'pointer' }}>
                  Annuler
                </button>
                <button type="submit" style={{ flex: 1, padding: '12px', borderRadius: '10px', border: 'none', backgroundColor: '#1e3a8a', color: '#ffffff', fontWeight: '700', fontSize: '13px', cursor: 'pointer' }}>
                  Enregistrer
                </button>
              </div>
            </form>
      ...
      {showClearModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(15, 23, 42, 0.6)', backdropFilter: 'blur(4px)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 1000, padding: '16px' }}>
          <div style={{ backgroundColor: '#ffffff', padding: '24px', borderRadius: '20px', maxWidth: '380px', width: '100%', textAlign: 'center', boxShadow: '0 20px 25px rgba(0,0,0,0.15)' }}>
            ...
            <div style={{ display: 'flex', gap: '10px' }}>
              <button onClick={() => setShowClearModal(false)} style={{ flex: 1, padding: '12px', borderRadius: '10px', border: '1px solid #cbd5e1', backgroundColor: '#ffffff', color: '#334155', fontWeight: '700', fontSize: '13px', cursor: 'pointer' }}>
                Annuler
              </button>
              <button onClick={confirmClearHistory} style={{ flex: 1, padding: '12px', borderRadius: '10px', border: 'none', backgroundColor: '#ef4444', color: '#ffffff', fontWeight: '700', fontSize: '13px', cursor: 'pointer' }}>
                Confirmer
              </button>
            </div>
          </div>
