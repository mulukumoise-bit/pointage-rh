...
export default function App() {
  ...
  const [currentLocation, setCurrentLocation] = useState('Recherche GPS...');

  useEffect(() => {
    const timer = setInterval(() => setCurrentTime(new Date()), 1000);
    const saved = localStorage.getItem('pointage_rh_gps_beige_v7');
    if (saved) {
      try { setRecords(JSON.parse(saved)); } catch (e) { console.error(e); }
    }
    const savedCompany = localStorage.getItem('pointage_rh_company_v7');
    if (savedCompany) {
      setCompany(savedCompany);
    }

    // Récupération de la position GPS au démarrage
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        ...
        () => {
          setCurrentLocation('Position non disponible');
        },
        { timeout: 10000, enableHighAccuracy: true }
      );
    } else {
      setCurrentLocation('GPS non supporté');
    }
  ...
  const handlePointage = (type) => {
    ...
    setRecords(updated);
    localStorage.setItem('pointage_rh_gps_beige_v7', JSON.stringify(updated));
    setName('');
    showToast(`Pointage (${type}) validé avec succès (GPS: ${currentLocation}).`);
  };
  ...
  const handleSaveCompany = (e) => {
    ...
    localStorage.setItem('pointage_rh_company_v7', newCompanyNameInput.trim());
  ...
  const confirmClearHistory = () => {
    setRecords([]);
    localStorage.removeItem('pointage_rh_gps_beige_v7');
    ...
    <div style={{ minHeight: '100vh', backgroundColor: '#f5f2eb', fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif', padding: '16px', boxSizing: 'border-box', color: '#1e293b' }}>
      <div style={{ maxWidth: '480px', margin: '0 auto' }}>
        ...
        <div style={{ backgroundColor: '#faf8f5', padding: '20px', borderRadius: '20px', marginBottom: '20px', textAlign: 'center', boxShadow: '0 4px 15px rgba(0,0,0,0.03)', border: '1px solid #e7e2d6' }}>
          ...
          <div style={{ marginTop: '10px', fontSize: '11px', color: '#059669', fontWeight: '700', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '4px' }}>
            <span>📍</span> GPS Actif : {currentLocation}
          </div>
        </div>
          ...
          {!isAdminUnlocked ? (
            ...
            <div>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                <div>
                  <h3 style={{ fontSize: '15px', margin: '0', color: '#1e293b', fontWeight: '800' }}>📊 Registre des Présences & GPS</h3>
                  ...
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px', textAlign: 'left' }}>
                    <thead>
                      <tr style={{ backgroundColor: '#1e3a8a', color: '#ffffff' }}>
                        ...
                        <th style={{ padding: '10px' }}>Heure & Date</th>
                      ...
                      {records.map((r, index) => (
                        <tr key={r.id} style={{ backgroundColor: index % 2 === 0 ? '#ffffff' : '#f5f2eb', borderBottom: '1px solid #e7e2d6' }}>
                          ...
                          <td style={{ padding: '10px', color: '#64748b' }}>
                            ...
                            <div style={{ fontSize: '10px' }}>{r.date}</div>
                          </td>
                          <td style={{ padding: '10px', color: '#059669', fontSize: '11px', fontFamily: 'monospace' }}>{r.location}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
          ...
          <div style={{ backgroundColor: '#faf8f5', padding: '24px', borderRadius: '20px', maxWidth: '380px', width: '100%', boxShadow: '0 20px 25px rgba(0,0,0,0.15)', border: '1px solid #e7e2d6' }}>
            ...
            <form onSubmit={handleSaveCompany}>
              ...
              <div style={{ display: 'flex', gap: '10px' }}>
                <button type="button" onClick={() => setIsEditingCompany(false)} style={{ flex: 1, padding: '12px', borderRadius: '10px', border: '1px solid #dcd5c7', backgroundColor: '#ffffff', color: '#334155', fontWeight: '700', fontSize: '13px', cursor: 'pointer' }}>
                  Annuler
                </button>
      ...
      {showClearModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(15, 23, 42, 0.6)', backdropFilter: 'blur(4px)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 1000, padding: '16px' }}>
          <div style={{ backgroundColor: '#faf8f5', padding: '24px', borderRadius: '20px', maxWidth: '380px', width: '100%', textAlign: 'center', boxShadow: '0 20px 25px rgba(0,0,0,0.15)', border: '1px solid #e7e2d6' }}>
            ...
            <div style={{ display: 'flex', gap: '10px' }}>
              <button onClick={() => setShowClearModal(false)} style={{ flex: 1, padding: '12px', borderRadius: '10px', border: '1px solid #dcd5c7', backgroundColor: '#ffffff', color: '#334155', fontWeight: '700', fontSize: '13px', cursor: 'pointer' }}>
                Annuler
